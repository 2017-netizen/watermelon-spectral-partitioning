function stage2_l600_four_band_nested_validation(subset)
% Repeated nested record-level CV for full-band versus S1-S4 comparisons.
...
logFile = fullfile(outDir, ['四波段重复嵌套交叉验证' suffix '_运行日志.txt']);
...
rangeMasks = {fullMask, find(wl >= 650 & wl <= 688), ...
    find(wl >= 690 & wl <= 746), find(wl >= 748 & wl <= 826), ...
    find(wl >= 828 & wl <= 950)};
...
pipelineNames = {'Full_AllBands', 'S1_AllBands', 'S2_AllBands', 'S3_AllBands', ...
    'S4_AllBands', 'Full_CARS', 'S1_CARS', 'S2_CARS', 'S3_CARS', 'S4_CARS'};
...
outXlsx = fullfile(outDir, ['四波段重复嵌套交叉验证结果' suffix '.xlsx']);
...
save(fullfile(outDir, ['四波段重复嵌套交叉验证结果' suffix '.mat']), 'cfg', 'wl', 'Y', ...
    'originalIndex', 'predCube', 'summaryTable', 'metricTable', 'pairTable', 'foldTable', '-v7.3');
summaryFile = fullfile(outDir, ['四波段重复嵌套交叉验证' suffix '_摘要.txt']);
